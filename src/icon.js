...
export class Icon extends IconBase {
  ...
  initialize(options) {
    ...
    opts.iconSize = iconSize
    opts.iconAnchor = Point.validate(opts.iconAnchor) ? new Point(opts.iconAnchor) : new Point([iconSize.x / 2, iconSize.y])
    
    if (opts.shadow === "ellipse") {
      // 30w 6h 
      opts.shadowSize = Point.validate(opts.shadowSize) ? new Point(opts.shadowSize) : new Point([iconSize.x, iconSize.x * 6 / 30])
      opts.shadowAnchor = Point.validate(opts.shadowAnchor) ? new Point(opts.shadowAnchor) : new Point([iconSize.x / 2, iconSize.x * 6 / 30 / 2])
    } else {
      // 39w 36h
      opts.shadowSize = Point.validate(opts.shadowSize) ? new Point(opts.shadowSize) : new Point([iconSize.x * 39 / 30, iconSize.x * 36 / 30])
      opts.shadowAnchor = Point.validate(opts.shadowAnchor) ? new Point(opts.shadowAnchor) : new Point([iconSize.x / 2, iconSize.x / 30 * 32])
    }
    opts.popupAnchor = Point.validate(opts.popupAnchor) ? new Point(opts.popupAnchor) : new Point([0, iconSize.y - iconSize.x + (iconSize.x / 2)])
    opts.tooltipAnchor = Point.validate(opts.tooltipAnchor) ? new Point(opts.tooltipAnchor) : new Point([0, iconSize.y - iconSize.x + (iconSize.x / 2)])
  }
